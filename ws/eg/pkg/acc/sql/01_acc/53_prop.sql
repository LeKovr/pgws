...
/* ------------------------------------------------------------------------- */
CREATE OR REPLACE FUNCTION prop_attr_account_abp(
  a_id   INTEGER
, a_code TEXT    DEFAULT NULL
) RETURNS SETOF cfg.prop_attr STABLE LANGUAGE 'sql' AS
$_$
-- a_id:   код владельца свойств
-- a_code: код свойства
...
/* ------------------------------------------------------------------------- */
CREATE OR REPLACE FUNCTION prop_attr_account_isv(
  a_id   INTEGER
, a_code TEXT    DEFAULT NULL
) RETURNS SETOF cfg.prop_attr STABLE LANGUAGE 'sql' AS
$_$
...
/* ------------------------------------------------------------------------- */
CREATE OR REPLACE FUNCTION prop_account_isv(
  a_id     INTEGER
...
-- a_prefix: часть кода свойства до '.'
  SELECT code, value FROM cfg.prop_value_list(acc.const_account_group_prop(), $1, CASE WHEN $2 IS NULL THEN 'isv' ELSE 'isv.' || $2 END, false)
$_$;
SELECT pg_c('f', 'prop_account_isv', 'Значения служебных свойств пользователя');

/* ------------------------------------------------------------------------- */
CREATE OR REPLACE FUNCTION prop_attr_team_abp(
  a_id   INTEGER
, a_code TEXT    DEFAULT NULL
) RETURNS SETOF cfg.prop_attr STABLE LANGUAGE 'sql' AS
$_$
...
/* ------------------------------------------------------------------------- */
CREATE OR REPLACE FUNCTION prop_attr_team_isv(
  a_id   INTEGER
, a_code TEXT    DEFAULT NULL
) RETURNS SETOF cfg.prop_attr STABLE LANGUAGE 'sql' AS
$_$
...
/* ------------------------------------------------------------------------- */
CREATE OR REPLACE FUNCTION prop_history_team(
  a_id   INTEGER
, a_code TEXT    DEFAULT NULL
) RETURNS SETOF cfg.prop_history STABLE LANGUAGE 'sql' AS
$_$
-- a_id:   код владельца свойств
-- a_code: код свойства
...
/* ------------------------------------------------------------------------- */
CREATE OR REPLACE FUNCTION prop_history_account(
  a_id   INTEGER
, a_code TEXT    DEFAULT NULL
) RETURNS SETOF cfg.prop_history STABLE LANGUAGE 'sql' AS
$_$
